...
import Socialslinks from "../Utlis/Socials";
  ...
  return (
    <div className=" relative h-auto w-auto flex flex-col">
      ...
      <div className="relative opacity-100 z-20 grid  items-center justify-items-center nike-container ">
        <div className=" grid items-center justify-items-center mt-3 md:mt-5">
          ...
          <div className="grid items-center gap-5 md:gap-3 absolute top-[33vh] w-auto h-auto lg:top-[37vh] left-[2%] xl:left-0"> {videos?.map((item, idex)=>(
            <Clip key={idex} imgsrc={item.imgsrc} clip={item.clip}/>

          ))} </div>
          <div className="grid items-center absolute top-[33vh] lg:top-[27vh] right-0 gap-3">
            {sociallinks?.map((val, i) =>(
             <Socialslinks key={i}   icon={val.icon} sociallinks={sociallinks}/>
            ))}
          </div>
        </div>
